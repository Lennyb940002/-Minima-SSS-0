...
export function StockView() {
  ...
  const loadData = async () => {
    try {
      ...
      console.log('Données reçues dans StockView:', stockData);
      setItems(stockData);
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        <div className="lg:col-span-3">
          <StockTable
            items={items}
            onUpdate={async (reference, data) => {
              await stockApi.updateStock(reference, data);
              await loadData();
            }}
            onDelete={async (reference) => {
              await stockApi.deleteStock(reference);
              await loadData();
            }}
          />
        </div>
